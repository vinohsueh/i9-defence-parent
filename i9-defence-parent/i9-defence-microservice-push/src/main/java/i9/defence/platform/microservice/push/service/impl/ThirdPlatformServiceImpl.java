        ...
        if (1 == project.getSendStatus()) {
            ...
            for (int i = 0; i < recipients.length; i++) {
                if (i == recipients.length - 1) {
                    clientNamesBuffer.append("{\"name\":\"").append(recipients[i]).append("\"," + "\"project\":\""
                            + project.getProjectName() + "\"," + "\"postion\":\"" + equipment.getRemarks() + "\"")
                            .append("}]");
            ...
            for (int h = 0; h < arr.length; h++) {
                ...
                if (0 == arr[h] && 1 == equipmentType) {
                    AliyunUtil.sendInfo(AliyunSMSEnum.WANING, clientPhonesBuffer.toString(),
                    ...
                    AliyunUtil.sendInfo(AliyunSMSEnum.HIDDENDANGER, clientPhonesBuffer.toString(),
                            clientNamesBuffer.toString(), clientSignNamesBuffer.toString());
                    // 2.7若SendType=0并且status=1则发送离线短信
                }
                if (1 == arr[h] && 0 == equipmentType) {
                    AliyunUtil.sendInfo(AliyunSMSEnum.OUTOFLINE, clientPhonesBuffer.toString(),
                            clientNamesBuffer.toString(), clientSignNamesBuffer.toString());
                }
