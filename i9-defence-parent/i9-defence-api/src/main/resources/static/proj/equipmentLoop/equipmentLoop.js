var equipmentLoopModule=angular.module('equipmentLoopModule',['ngAnimate','ui.bootstrap','app']);
var equipmentLoopService = equipmentLoopModule.factory('equipmentLoopService',
...
var equipmentLoopNgControl=equipmentLoopModule.controller('equipmentLoopNgControl',function($rootScope, $scope,$stateParams,  $log, $http, $window, $state,$modal, toaster,equipmentLoopService,httpService){
	//分页条件
	$scope.pageSize = 10;
	$scope.currentPage = 1;
	//初始化
	$scope.initTable = function (){
		...
		httpService.post({url:'./equipmentLoop/pageEquipmentLoop',data:pageParam,showSuccessMsg:false}).then(function(data) {  
			$scope.equipmentLoops = data.data.data.pageList;
